...
using UnityEngine;
using DG.Tweening;
public class PlataformaMovimentoHorizontal : MonoBehaviour {    
    ...
    private void Start(){
        ...
        moveSequence.Append(transform.DOMoveX(initialPosition.x + moveDistance, moveDuration).SetEase(Ease.Linear));

        // Pausa por 1.5 segundos
        moveSequence.AppendInterval(pauseDuration);

        // Move a plataforma de volta para a posição inicial
        moveSequence.Append(transform.DOMoveX(initialPosition.x, moveDuration).SetEase(Ease.Linear));
